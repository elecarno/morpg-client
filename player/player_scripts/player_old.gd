...
onready var safe_to_stand = get_node("safe_to_stand")
...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_pressed("crouch") and !is_falling:
		is_crouching = true
		is_sprinting = false
	else:
		if !safe_to_stand.is_colliding():
			is_crouching = false
	...
	elif is_crouching and !is_dashing:
		if is_running and max_speed != max_sprint_speed:
			max_speed = max_run_speed
		else:
			max_speed = max_sprint_speed
		accel = 0
	...
	if is_jumping:
		...
		_anim.play("dash")
	elif is_crouching or safe_to_stand.is_colliding():
		_anim.play("slide")
	elif is_running:
		if is_sprinting:
			_anim.play("sprint")
		elif safe_to_stand.is_colliding():
